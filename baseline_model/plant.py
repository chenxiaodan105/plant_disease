...
def train(train_dir, annotations, max_step, checkpoint_dir='./checkpoint/'):
    ...
    with tf.Session() as sess:
        ...
        val_data = plant_input.plant_data_fn('./Validation_images','./validation_labels.json')
        x_val,y_val = val_data.next_batch(1000,IMAGE_SIZE)
        for step in range(start_step, start_step + max_step):
            ...
            if step % 50 == 0:
                train_accuracy = sess.run(accuracy, feed_dict={features: x_val, labels: y_val, keep_prob: 1})
                train_loss = sess.run(cross_entropy, feed_dict={features: x_val, labels: y_val, keep_prob: 1})
